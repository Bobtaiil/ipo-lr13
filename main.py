...
from tkinter import ttk  #Импортируем модуль ttk
from tkinter.messagebox import showerror  #Импортируем showerror
from tkinter import filedialog as fd  #Импортируем filedialog
from tkinter import simpledialog  #Импортируем simpledialog
from PIL import Image, ImageTk, ImageFilter, ImageFont, ImageDraw  #Импортируем модули из библиотеки PIL
...
class ImageHandler:  #Определяем класс ImageHandler
    def __init__(self):  #Инициализация класса
        self.img = None  #Хранение изображения
        self.file_path = None  #Хранение пути к файлу

    def load_image(self, file_path):  #загрузка изображения
        self.img = Image.open(file_path)  #Открываем изображение

    def re_size(self):  #Редакция размеров
        max_size = (200, 200)  #Устанавливаем максимальный размер
        self.img.thumbnail(max_size) 

    def save_to_file(self, file_path, name):  #сохраняем в файл
        self.img.save(file_path)  #Сохраняем изображение по указанному пути
        new_file_path = f"{name}.png"  #ренейм
        os.rename(file_path, new_file_path)

    def countor_load(self):  #Контур
        self.img = self.img.filter(ImageFilter.CONTOUR)

    def text_var_load(self):  #Добавление текста
        size = self.img.size  #Размеры
        draw = ImageDraw.Draw(self.img)  #Создаем объект для рисования на изображении
        font = ImageFont.truetype("arial.ttf", 24) #Загрузка шрифта
        text = "Вариант 3"  
        bbox = draw.textbbox((0, 0), text, font=font)  #Размер текста
        text_width, text_height = bbox[2] - bbox[0], bbox[3] - bbox[1]  #ширина и высота текста
        text_x = (size[0] - text_width) / 2  #централизуем текст
        text_y = (size[1] - text_height) / 2  
        draw.text((text_x, text_y), text, font = font)  #Рисуем текст на изображении

class Imagine(tk.Frame, ImageHandler):  #Определяем класс Imagine
